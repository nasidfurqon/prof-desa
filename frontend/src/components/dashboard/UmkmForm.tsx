...
import { umkmSchema, UmkmFormSchema } from "../../lib/schemas";
...
interface UmkmFormProps {
  initialValues?: Umkm;
  onSubmit: (values: UmkmFormSchema) => void;
  isSubmitting: boolean;
}

export function UmkmForm({ initialValues, onSubmit, isSubmitting }: UmkmFormProps) {
  const {
    register,
    handleSubmit,
    control,
    formState: { errors },
  } = useForm<UmkmFormSchema>({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        <FormLabel required>Nama UMKM</FormLabel>
      ...
      <div>
        ...
        <input {...register("ownerName")} className={inputClass(Boolean(errors.ownerName))} />
